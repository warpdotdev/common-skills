...
Do not include `Other`, `Other...`, or any equivalent custom-instructions choice in `ask_user_question` options. The `ask_user_question` UI already provides a built-in freeform Other control because `SupportsOther` is enabled, so listing another Other option creates duplicate Other entries. When custom instructions are needed, rely on the built-in Other control instead of adding an option generated by the LLM.
