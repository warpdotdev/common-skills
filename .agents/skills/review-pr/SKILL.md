...
- **Regression-test adequacy**: For any test the PR frames as a regression test (by name, docstring, or PR description), find the original bug's real call site — the code path that actually crashed or misbehaved. Merely confirming the test *executes* that path is not enough: name the specific assertion or mechanism in the test that would *fail* if the exact buggy code were restored at that call site. An assertion the old, buggy implementation would also have satisfied leaves the regression undetected even though the test technically runs through the right path. If you cannot name that failing assertion/mechanism, flag the test as inadequate and say what it should assert on instead. Reaching only a private helper or an internal function one step removed from the real call site is one common way this fails — not the only one.
