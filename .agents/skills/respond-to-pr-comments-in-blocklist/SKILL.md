...
Do not include `Other`, `Other...`, or any equivalent custom-response choice in `ask_user_question` options. The `ask_user_question` UI already provides a built-in freeform Other control because `SupportsOther` is enabled, so listing another Other option creates duplicate Other entries. When the user needs a custom mode, response, rationale, posting instruction, or next step, rely on the built-in Other control instead of adding an option generated by the LLM.
...
Use the built-in Other control's freeform field for custom responses or approaches. Do not return control to the user in normal chat solely to collect custom freeform text.
...
2. Ask for or propose a concise commit message if one is not already clear; rely on the built-in Other control for custom commit instructions.
